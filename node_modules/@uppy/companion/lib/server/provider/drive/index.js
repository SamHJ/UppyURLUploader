...
class Drive extends Provider {
    constructor(options) {
        ...
        this.client = purest(Object.assign(Object.assign({}, options), { provider: Drive.authProvider, alias: 'drive', version: 'v3' }));
    }
                ...
                return __awaiter(this, void 0, void 0, function* () {
                    try {
                        ...
                        if (!shouldListSharedDrives)
                            return undefined;
                        const resp = yield new Promise((resolve, reject) => client
                            .get('drives')
                            .qs({ fields: SHARED_DRIVE_FIELDS })
                            .auth(options.token)
                            .request((err, resp2) => {
                            if (err || resp2.statusCode !== 200)
                                return reject(handleErrorResponse(err, resp2));
    ...
    stats({ id, token }) {
        return __awaiter(this, void 0, void 0, function* () {
            const getStats = (statsOfId) => __awaiter(this, void 0, void 0, function* () {
                return new Promise((resolve, reject) => {
                    this.client
                        .query()
                        ...
                        if (err || resp.statusCode !== 200)
                            return reject(this._error.bind(this)(err, resp));
                        return resolve(resp.body);
                    });
                });
            });
            let stats = yield getStats(id);
            // If it is a shortcut, we need to get stats again on the target
            if (adapter.isShortcut(stats.mimeType)) {
                stats = yield getStats(stats.shortcutDetails.targetId);
            }
            return stats;
        });
    }
            ...
            requestStream
                ...
                if (resp.statusCode !== 200) {
                    waitForFailedResponse(resp)
                        .then((jsonResp) => {
                        onData(this._error(null, Object.assign(Object.assign({}, resp), { body: jsonResp })));
                    })
    ...
    _size({ id, token }) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const body = yield this.stats({ id, token });
